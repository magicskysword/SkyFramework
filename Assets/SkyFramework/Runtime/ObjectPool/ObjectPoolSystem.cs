...
{
    public class ObjectPoolSystem : SingletonMono<ObjectPoolSystem>
    {
        private Dictionary<string, SubObjectPool> pools = new Dictionary<string, SubObjectPool>();

        public bool CreatePool(string id, IReuseable prefab)
        {
        ...
        public bool CreatePool(string path)
        {
            ...
            GameObject go = ResourcesSystem.Instance.LoadAsset<GameObject>(path);
            if (go == null)
            {
                LogError($"创建对象池失败，资源 {path} 不存在");
            }

            IReuseable prefab = go.GetComponent<IReuseable>();
            ...
            pool.transform.parent = transform;
            pools.Add(path, pool);
            ObjectPoolSystem.Log($"对象池 {path} 创建成功");
            return true;
        }
        ...
        public void CreatePoolAsync(string path,Action<SubObjectPool> callback = null)
        {
            ...
            ResourcesSystem.Instance.LoadAssetAsync<GameObject>(path, go =>
            {
        ...
        public IReuseable Spawn(string id)
        {
            if (!pools.TryGetValue(id, out var pool))
            {
        ...
        /// <returns></returns>
        public IReuseable Spawn(string id,IReuseable prefab)
        {
            if (!pools.TryGetValue(id, out var pool))
            {
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public IReuseable SpawnByPath(string path)
        {
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public void SpawnByPathAsync(string path,Action<IReuseable> callback)
        {
            if (!pools.TryGetValue(path, out var pool))
            {
        ...
        public bool Unspawn(string id, IReuseable go)
        {
            if (!pools.TryGetValue(id, out var pool))
            {
        ...
        {
            Debug.Log($"<color=cyan>[{nameof(ObjectPoolSystem)}]</color>{msg}");
        }
        
        public static void LogWarning(string msg)
        {
            Debug.LogWarning($"<color=cyan>[{nameof(ObjectPoolSystem)}]</color>{msg}");
        }
        
        public static void LogError(string msg)
        {
            Debug.LogError($"<color=cyan>[{nameof(ObjectPoolSystem)}]</color>{msg}");
        }
    }
